# Session journal index

| Date | Session | Model | Input | Output | Cache read | Cache write | Total | Focus |
|---|---|---|---|---|---|---|---|---|
| 2026-07-17 | 01 | Claude Fable 5 | _pending_ | _pending_ | _pending_ | _pending_ | _pending_ | M1 — vault + export core (classifier outage: token counts backfill pending) |
| 2026-07-17 | 02 | Claude Opus 4.8 | 1,125 | 132,850 | 7,834,466 | 150,116 | 8,118,557 | Repo restructure (_schema/_data, validation/, data-purity hook) + M3 RoleFraming schema slice + ADR-0001 |
| 2026-07-18 | 03 | Claude Opus 4.8 | 113 | 105,673 | 6,708,144 | 339,811 | 7,153,741 | Ingest 20 GitHub repos as Projects; promote 5 rescued stub skills; add rg:lastActivity recency; commit pending interview + ingestion (3 domain-pure) |
| 2026-07-19 | 04 | Claude Opus 4.8 | 142 | 87,233 | 7,209,312 | 152,617 | 7,449,304 | M2 — JSON Resume + PDF exports, standalone graph.html preview viewer (not M4/M5 site) |
| 2026-07-20 | 05 | Claude Opus 4.8 | 238 | 85,623 | 16,392,056 | 199,529 | 16,677,446 | WS5 work-project → bullet migration (9 engagements, 21 skills repointed); make all green for the first time since WS2 (project.py certifies-channel bug); 7/24 credential URLs backfilled |
| 2026-07-23 | 06 | Claude Opus 4.8 | 4,311 | 15,427 | 504,451 | 81,391 | 605,580 | WS9 — architecture sequence diagrams (Mermaid: make-all gates, projection stages, publishing trust boundary); push backlog + republish general & applications |
