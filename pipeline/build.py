...
def export_graph(vault: Path, build_dir: Path, strict: bool = True) -> Graph:
    """Run the vendored exporter; return schema + data layers merged into one Graph.

    The exporter's raw two-layer output (schema.ttl + data.ttl) is intermediate,
    not a distributable: it stages under build/layers/, never in dist/. Only the
    merged graph this returns becomes a dist artifact (via write_outputs)."""
    layers_dir = build_dir / "layers"
    layers_dir.mkdir(parents=True, exist_ok=True)
    proc = subprocess.run(
        [sys.executable, str(VENDORED_EXPORTER), str(vault), "--out-dir", str(layers_dir)],
        capture_output=True,
        text=True,
    )
    if proc.stdout:
    ...
    graph = Graph()
    graph.parse(layers_dir / "schema.ttl", format="turtle")
    graph.parse(layers_dir / "data.ttl", format="turtle")
    return graph
...
def main() -> int:
    ...
    parser.add_argument("--out", type=Path, default=REPO_ROOT / "dist",
                        help="distributables root (graph.ttl + graph.jsonld)")
    parser.add_argument("--build-dir", type=Path, default=REPO_ROOT / "build",
                        help="intermediate exporter staging (build/layers/); disposable, never published")
    parser.add_argument("--no-strict", action="store_true",
                        help="report structural warnings without failing the build")
    args = parser.parse_args()

    try:
        graph = export_graph(args.vault, args.build_dir, strict=not args.no_strict)
